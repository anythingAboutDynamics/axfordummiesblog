	...
	<li class="<?php echo $oddcomment; ?>"
		...
		<p class="ListDate">
			<a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('d/m/Y') ?>
				as <?php comment_time() ?> <?php edit_comment_link(__('&nbsp;&nbsp;<strong>Editar coment&aacute;rio</strong>','ax4dummies'),'',''); ?>
			</a>
		</p> <span class="ListNr"><?php echo get_avatar($comment->comment_author_email ,33); ?>
			<?php //$commentNumber++; echo $commentNumber; ?> </span> <?php comment_text() ?>
		<?php if ($comment->comment_approved == '0') : ?> <?php _e('Seu coment&aacute;rio est&aacute; em modera&ccedil;&atilde;o.','ax4dummies');?>
...
<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php"
	...
	<p>
		...
		&nbsp;<a
			...
			title="<?php _e('Logout dessa conta','ax4dummies') ?>">Logout &rarr;</a>
	</p>
	<?php else : ?>

	<p>
		<label for="author"> <input type="text" name="author" id="author"
			value="<?php echo $comment_author; ?>" tabindex="1" class="TextField"
			style="width: 210px;" />Nome <small><?php if ($req) _e('(requerido)','ax4dummies'); ?>
		</small>
		</label>
	</p>
