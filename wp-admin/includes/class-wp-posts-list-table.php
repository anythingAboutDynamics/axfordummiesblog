...
<tr id="post-<?php echo $post->ID; ?>"
	...
	foreach ( $columns as $column_name => $column_display_name ) {
		...
		switch ( $column_name ) {
			...
			case 'cb':
				?>
	<th scope="row" class="check-column"><?php if ( $can_edit_post ) { ?><input
		type="checkbox" name="post[]" value="<?php the_ID(); ?>" /> <?php } ?>
	</th>
	<?php
	break;
...
case 'title':
	if ( $this->hierarchical_display ) {
		...
		?>
	<td <?php echo $attributes ?>><strong><?php if ( $can_edit_post && $post->post_status != 'trash' ) { ?><a
			class="row-title" href="<?php echo $edit_link; ?>"
			title="<?php echo esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;' ), $title ) ); ?>"><?php echo $pad; echo $title ?>
		</a> <?php } else { echo $pad; echo $title; 
		}; _post_states( $post ); echo isset( $parent_name ) ? ' | ' . $post_type_object->labels->parent_item_colon . ' ' . esc_html( $parent_name ) : ''; ?>
	</strong> <?php
	}
	else {
		$attributes = 'class="post-title page-title column-title"' . $style;
	...
	<td <?php echo $attributes ?>><strong><?php if ( $can_edit_post && $post->post_status != 'trash' ) { ?><a
			class="row-title" href="<?php echo $edit_link; ?>"
			title="<?php echo esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;' ), $title ) ); ?>"><?php echo $title ?>
		</a> <?php } else { echo $title; 
		}; _post_states( $post ); ?>
	</strong> <?php
	if ( 'excerpt' == $mode ) {
		the_excerpt();
...
<form method="get" action="">
	<table style="display: none">
		...
		if ( $bulk ) : ?>
							...
							</label> <label> <span class="title"><?php _e( 'Slug' ); ?> </span>
								<span class="input-text-wrap"><input type="text"
									name="post_name" value="" /> </span>
							</label>
	...
	?>
							...
							<div class="inline-edit-group">
								<label class="alignleft"> <span class="title"><?php _e( 'Password' ); ?>
										...
										value="" /> </span>
								</label> <em style="margin: 5px 10px 0 0" class="alignleft"> <?php
								...
								?>
									...
									class="checkbox-title"><?php echo __( 'Private' ); ?> </span>
								</label>
							</div>
					...
					<fieldset class="inline-edit-col-center inline-edit-categories">
						<div class="inline-edit-col">
							...
							<span class="title inline-edit-categories-label"><?php echo esc_html( $taxonomy->labels->name ) ?>
								<span class="catshow"><?php _e( '[more]' ); ?> </span> <span
								class="cathide" style="display: none;"><?php _e( '[less]' ); ?>
		...
		if ( $bulk ) : ?>
			...
			$all_post_formats = get_post_format_strings(); ?>
							<div class="inline-edit-group">
								<label class="alignleft" for="post_format"> <span class="title"><?php _e( 'Post Format' ); ?>
										...
										<?php endif; ?>
										<option value="0">
											<?php _ex( 'Standard', 'Post format' ); ?>
										</option>
										<?php foreach ( $all_post_formats as $slug => $format ):
					if ( $slug != 'standard' ) : ?>
										<option value="<?php echo esc_attr( $slug ); ?>"
										<?php if ( ! in_array( $slug, $post_formats[0] ) ) echo ' class="unsupported"'; ?>>
				...
				?>
						<a accesskey="s" href="#inline-edit"
							title="<?php esc_attr_e( 'Update' ); ?>"
							class="button-primary save alignright"><?php echo esc_attr( $update_text ); ?>
